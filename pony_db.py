...
class User(db.Entity):
	...
	total_snatches = Required(int, default=0, min=0)
	last_gift_datetime = Required(datetime, default=datetime(year=2001,month=9,day=11,hour=8,minute=46))

	def get_stats(self):
		return self.total_presents, self.stolen_presents, self.given_presents, self.total_snatches, self.given_presents, "Ready for presents" if self.last_gift_datetime < datetime.now() - timedelta(hours=1) else "Santa needs a break from yo needy ass"
